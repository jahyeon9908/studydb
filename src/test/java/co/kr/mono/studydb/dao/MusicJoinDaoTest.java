...
class MusicJoinDaoTest {
    ...
    void selectAlbumByPrimaryKeyTest(){
        List<Album> album = musicJoinDao.selectAlbumByPrimaryKey1("BS1");
        // album.forEach(System.out::println);
        System.out.println(musicJoinDao);
    }

    @DisplayName("[Association Mappin & Collection Mapping] ")
    @Test
    void selectAlbumByPrimaryKey2Test(){
        List<Album> album = musicJoinDao.selectAlbumByPrimaryKey2("BS1");
        // album.forEach(System.out::println);
    ...
    @DisplayName("[AllSong] ")
    @Test
    void selectAllSongTest(){
        List<Artist> artists = musicJoinDao.selectAllSong("BTS");
        // album.forEach(System.out::println);
